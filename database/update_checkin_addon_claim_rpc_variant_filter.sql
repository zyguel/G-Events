-- Migration: Extend add-on claim RPC with optional variant filter.
-- This allows scanner/station flows to claim only one add-on variant per scan
-- while preserving existing "claim all" behavior when no variant filter is passed.

DROP FUNCTION IF EXISTS public.claim_registration_addons_atomic(integer, integer, text, text, integer);
...
CREATE OR REPLACE FUNCTION public.claim_registration_addons_atomic(
    ...
    p_add_on_variant_id integer DEFAULT NULL
)
RETURNS TABLE (
...
BEGIN
    ...
    WITH locked_entitlements AS (
        ...
        WHERE e.registration_id = p_registration_id
          AND (p_add_on_variant_id IS NULL OR e.add_on_variant_id = p_add_on_variant_id)
        FOR UPDATE OF e, v
    ),
    claimable AS (
        SELECT
...
REVOKE ALL ON FUNCTION public.claim_registration_addons_atomic(integer, integer, text, text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.claim_registration_addons_atomic(integer, integer, text, text, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.claim_registration_addons_atomic(integer, integer, text, text, integer) TO service_role;
