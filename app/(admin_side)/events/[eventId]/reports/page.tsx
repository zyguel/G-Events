...
// Event name mapping
const eventNames: Record<string, { name: string; date: string }> = {
    'devfest-2025': { name: 'DevFest Cebu 2025', date: 'July 22, 2025' },
    'wtm-2025': { name: 'Women Techmakers 2025', date: 'March 8, 2025' },
    'io-extended-2024': { name: 'Google I/O Extended 2024', date: 'June 15, 2024' },
};
...
export default function EventReportsPage() {
    ...
    // Get event info
    const eventInfo = eventNames[eventId] || { name: eventId, date: '2025' };
    const sidebarEvent = {
        id: eventId,
        name: eventInfo.name,
        date: eventInfo.date,
    ...
    const handleExport = useCallback(async (format: string) => {
        ...
        const filename = `${eventInfo.name.replace(/\s+/g, '_')}_${activeTab}_Report_${timestamp}`;
        ...
        try {
            if (format === 'csv') {
                ...
                doc.text(`${eventInfo.name} - ${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Report`, pageWidth / 2, 20, { align: 'center' });
                doc.setFontSize(10);
    ...
    }, [activeTab, eventInfo.name, registrants, breakoutSessions]);
