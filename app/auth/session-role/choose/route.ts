...
export async function POST(request: Request) {
  ...
  if (role !== SESSION_ROLE.ATTENDEE && role !== SESSION_ROLE.ORGANIZER) {
    const selectRoleUrl = new URL('/auth/session-role', requestUrl.origin);
  ...
  if (role === SESSION_ROLE.ATTENDEE) {
    const attendeeUrl = new URL('/home', requestUrl.origin);
  ...
  if (context.memberships.length === 0) {
    const noOrgUrl = new URL('/auth/session-role', requestUrl.origin);
  ...
  if (!hasExplicitSelection && context.memberships.length > 1) {
    const chooseOrganizationUrl = new URL('/auth/session-role/organization', requestUrl.origin);
